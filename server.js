        ...
        if (req.method === 'POST' && pathname === '/resume_creator/preview') {
            ...
            req.on('end', () => {
                ...
                app.renderToHTML(req, res, '/preview', buffer).then(data => {
                    const dir = '.' + data.match(/\/_next\/static\/css\/styles.\w*.?chunk.css/)[0];
                    if (buffer.type === 'pdf') {
                        data = staticHtmlFormatter(
                            data,
                            dir,
                            '100%'
                        );
                    ...
                    if (buffer.type === 'html') {
                        ...
                        data = data.replace(
                            /\<img src="data:image\/(\w+);base64,([\s\S]+?)"/,
                            (all, type, base64) => {
                                const _base64 = Buffer.from(base64, 'base64').toString(
                                    'base64'
                                );
